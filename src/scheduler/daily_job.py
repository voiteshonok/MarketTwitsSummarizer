...
class DailyJobScheduler:
    ...
    def start_scheduler(self):
        """Start the scheduler."""
        try:
            # Schedule news dump job at 15:00 UTC+3
            self.scheduler.add_job(
                self.dump_news_job,
                trigger=CronTrigger(hour=16, minute=16, timezone=config.SCHEDULER_TIMEZONE),
                id="dump_news_job",
                name="Daily News Dump",
                replace_existing=True
            )
            
            # Schedule summary push job at 15:07 UTC+3
            self.scheduler.add_job(
                self.push_summary_job,
                trigger=CronTrigger(hour=16, minute=17, timezone=config.SCHEDULER_TIMEZONE),
                id="push_summary_job",
